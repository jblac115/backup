...
if (totalUsage >= titanDistance) {
    ...
    if (daysOxygen >= travelTime) {
        ...
        let Food = 270;  // Servings of healthy food per person (One Meal)
        let foodSpoiled = false;    // Is the food spoiled?
	let foodDay = Food / 3	// Each crew member gets 3 servings of food per day
        let foodTotal = foodDay / totalCrew;   // How many days of food on-board
        
        console.log("You have " + Food + " servings of food on-board");
        console.log("You have " + foodTotal + " days of food on-board");

        // If there are too many days worth of food on-board, some of it will go bad.
        if (foodTotal > 40) {
            foodSpoiled++;
        }
