...
gameScene.update = function(){
  ...
  if (cursors.left.isDown && this.player.x>20){
    ...
    else if (cursors.right.isDown && this.player.x<560)
    {
        this.player.x += this.playerSpeed;
    }
    else if (cursors.down.isDown && this.player.y<285){
        this.player.y += this.playerSpeed;
    }
    else if (cursors.up.isDown && this.player.y>70)
    {
        this.player.y-=this.playerSpeed;
    }
    // use WASD
  ...
  for(let i = 0; i< numEnemies; i++) {
    ...
    if(conditionUp || conditionDown) {
      enemies[i].speed *= -1;
      if(this.player.x>enemies[i].x){
        this.enemies.key='treasure.png'
      }
    }
